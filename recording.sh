...
function sleep_until_next_even_minute() {
    ...
    echo ${wakeup_time}
    sleep_seconds=$((${sleep_seconds}-2))  # PA0SIM
    sleep ${sleep_seconds}
}
...
function audio_recording_daemon() 
{
    ...
    while true; do
        ...
        ##sox -q -t  --type --channels 1 alsa hw:${audio_device},${audio_subdevice} --rate 12k ${wav_file_name} trim 0 ${capture_secs} ${SOX_MIX_OPTIONS-}
        sox -t alsa plughw:CARD=Loopback,DEV=1 --rate 12k -c 1 --bits 16 ${wav_file_name} trim 0 ${capture_secs} ${SOX_MIX_OPTIONS-}
        #sox -t alsa hw:1,1 --rate 12k -c 1 --bits 16 ${wav_file_name} trim 0 ${capture_secs} ${SOX_MIX_OPTIONS-}
